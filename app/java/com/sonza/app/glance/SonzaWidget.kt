...
class SonzaWidget : GlanceAppWidget() {
                ...
                LaunchedEffect(currentSong?.id) {
                    if (currentSong?.thumbnailUrl != null) {
                        try {
                            ...
                            android.util.Log.w("SonzaWidget", "Artwork load failed: ${e.message}")
                        }
                    }
                }
                    ...
                    Column(
                        ...
                        Row(
                            ...
                            Column(
                                ...
                                Text(
                                    text = currentSong?.artist ?: "Sonza Player",
                                    style = TextStyle(
...
class PlayPauseAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        val entryPoint = EntryPointAccessors.fromApplication(context, SonzaWidget.WidgetEntryPoint::class.java)
        entryPoint.getMusicPlayer().togglePlayPause()
        SonzaWidget().update(context, glanceId)
    }
}
