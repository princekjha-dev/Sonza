...
class MusicPlayerService : MediaLibraryService() {
    ...
    @Inject
    @com.sonza.app.di.PlayerDataSource
    ...
    @Inject
    lateinit var spatialAudioProcessor: com.sonza.app.player.SpatialAudioProcessor

    @Inject
    lateinit var loudnessAnalyzer: com.sonza.app.player.LoudnessAnalyzer

    @Inject
    lateinit var sleepTimerManager: com.sonza.app.player.SleepTimerManager
    ...
    private var cachedHomeSections: List<com.sonza.app.core.model.HomeSection> = emptyList()
    
    // Cache for search results so onSearch & onGetSearchResult stay consistent
    private var cachedSearchQuery: String? = null
    private var cachedSearchResults: List<com.sonza.app.core.model.Song> = emptyList()
    
    // Browse-level song cache: videoId -> Song, populated by createPlayableMediaItem
    private val cachedBrowseSongs: MutableMap<String, com.sonza.app.core.model.Song> = java.util.concurrent.ConcurrentHashMap()
    ...
    private val playlistContextCache: MutableMap<String, List<com.sonza.app.core.model.Song>> = java.util.concurrent.ConcurrentHashMap()
    ...
    override fun onCreate() {
        ...
        val loadControl = androidx.media3.exoplayer.DefaultLoadControl.Builder()
            .setBufferDurationsMs(
                10_000,  // minBufferMs
                50_000,  // maxBufferMs
                2_000,   // bufferForPlaybackMs — start playback ASAP; Fix #3 handles buffer-low slowdown
            ...
            val transitionListener = object : androidx.media3.common.Player.Listener {
                ...
                override fun onPlayWhenReadyChanged(playWhenReady: Boolean, reason: Int) {
                    ...
                    if (!playWhenReady && (reason == Player.PLAY_WHEN_READY_CHANGE_REASON_USER_REQUEST || reason == Player.PLAY_WHEN_READY_CHANGE_REASON_REMOTE)) {
                        ...
                        android.util.Log.d("MusicPlayerService", "AudioFocusLoss change: playWhenReady=$playWhenReady, wasPlayingBeforeInterruption=$wasPlayingBeforeInterruption")
                        if (!playWhenReady) {
                            wasPlayingBeforeInterruption = true
                ...
                override fun onPlaybackSuppressionReasonChanged(playbackSuppressionReason: Int) {
                    ...
                    if (playbackSuppressionReason == Player.PLAYBACK_SUPPRESSION_REASON_TRANSIENT_AUDIO_FOCUS_LOSS) {
                        ...
                        val isPlaying = mediaLibrarySession?.player?.isPlaying == true || mediaLibrarySession?.player?.playWhenReady == true
                        if (isPlaying) {
                            wasPlayingBeforeInterruption = true
                            pausedForTransientFocus = true
                            wasPlayingBeforeSuppression = true
            ...
            override fun onGetChildren(session: MediaLibrarySession, browser: MediaSession.ControllerInfo, parentId: String, page: Int, pageSize: Int, params: LibraryParams?): com.google.common.util.concurrent.ListenableFuture<LibraryResult<ImmutableList<MediaItem>>> {
                ...
                serviceScope.launch {
                    try {
                        ...
                        when (parentId) {
                            ROOT_ID -> {
                                children.add(createBrowsableMediaItem(HOME_ID, "Home", iconResId = com.sonza.app.R.drawable.ic_music_note))
                                children.add(createBrowsableMediaItem(LIBRARY_ID, "Library", iconResId = com.sonza.app.R.drawable.ic_heart_outline))
                                children.add(createBrowsableMediaItem(ARTISTS_ID, "Artists", iconResId = com.sonza.app.R.drawable.ic_music_note, mediaType = MediaMetadata.MEDIA_TYPE_FOLDER_ARTISTS))
                                children.add(createBrowsableMediaItem(ALBUMS_ID, "Albums", iconResId = com.sonza.app.R.drawable.ic_music_note, mediaType = MediaMetadata.MEDIA_TYPE_FOLDER_ALBUMS))
                                children.add(createBrowsableMediaItem(DOWNLOADS_ID, "Downloads", iconResId = com.sonza.app.R.drawable.ic_music_note))
                                children.add(createBrowsableMediaItem(LOCAL_ID, "Local Music", iconResId = com.sonza.app.R.drawable.ic_music_note))
                            }
                            HOME_ID -> {
                            ...
                            LIBRARY_ID -> {
                                children.add(createBrowsableMediaItem(LIKED_SONGS_ID, "Liked Songs", iconResId = com.sonza.app.R.drawable.ic_heart, mediaType = MediaMetadata.MEDIA_TYPE_PLAYLIST))
                                children.add(createBrowsableMediaItem(PLAYLISTS_ID, "Your Playlists", iconResId = com.sonza.app.R.drawable.ic_music_note, mediaType = MediaMetadata.MEDIA_TYPE_FOLDER_PLAYLISTS))
                            }
                            ARTISTS_ID -> {
                                ...
                                if (parentId.startsWith("section_")) {
                                    ...
                                    if (index != null && index in cachedHomeSections.indices) {
                                        ...
                                        val sectionSongs = section.items
                                            .filterIsInstance<com.sonza.app.core.model.HomeItem.SongItem>()
                                            .map { it.song }
                                        ...
                                        section.items.forEach { homeItem ->
                                             if (homeItem is com.sonza.app.core.model.HomeItem.SongItem) {
                                                 children.add(createPlayableMediaItem(homeItem.song))
                                             } else if (homeItem is com.sonza.app.core.model.HomeItem.PlaylistItem) {
                                             ...
                                             } else if (homeItem is com.sonza.app.core.model.HomeItem.AlbumItem) {
                                    ...
                                    val playlist = if (isAutoMix) {
                                        kotlinx.coroutines.withTimeoutOrNull(8_000L) {
                                            ...
                                            com.sonza.app.core.model.Playlist(playlistId, resolveAutoMixTitle(playlistId), "YouTube Music", null, emptyList())
                                        }
                                    } else {
                        ...
                        if (lastState != null) {
                            ...
                            for (i in 0 until jsonArray.length()) {
                                ...
                                val song = com.sonza.app.core.model.Song(
                                    ...
                                    source = try {
                                        com.sonza.app.core.model.SongSource.valueOf(obj.optString("source", "YOUTUBE"))
                                    } catch (e: Exception) {
    ...
    private fun getCustomLayout(): List<CommandButton> {
        ...
        val likeIcon = if (isCurrentSongLiked) com.sonza.app.R.drawable.ic_heart else com.sonza.app.R.drawable.ic_heart_outline
        
        val shuffleIcon = if (isShuffleOn) {
            com.sonza.app.R.drawable.ic_shuffle_on
        } else {
            com.sonza.app.R.drawable.ic_shuffle
        }

        val repeatIcon = when (player?.repeatMode) {
            Player.REPEAT_MODE_ALL -> com.sonza.app.R.drawable.ic_repeat_all_on
            Player.REPEAT_MODE_ONE -> com.sonza.app.R.drawable.ic_repeat_one_on
            else -> com.sonza.app.R.drawable.ic_repeat 
        }
        return listOf(
            CommandButton.Builder()
                .setDisplayName(getString(com.sonza.app.R.string.notification_action_shuffle))
                .setSessionCommand(SessionCommand(COMMAND_SHUFFLE, android.os.Bundle.EMPTY))
                .setIconResId(shuffleIcon)
                .build(),
            CommandButton.Builder()
                .setDisplayName(getString(com.sonza.app.R.string.notification_action_like))
                .setSessionCommand(SessionCommand(COMMAND_LIKE, android.os.Bundle.EMPTY))
                .setIconResId(likeIcon)
                .build(),
            CommandButton.Builder()
                .setDisplayName(getString(com.sonza.app.R.string.notification_action_repeat))
            ...
            CommandButton.Builder()
                ...
                .setIconResId(com.sonza.app.R.drawable.ic_play)
                .build()
        )
    }
    ...
    private fun resolveAutoMixTitle(playlistId: String): String =
        com.sonza.app.player.AutoMix.resolveTitle(playlistId)
    ...
    private fun createPlayableMediaItem(song: com.sonza.app.core.model.Song): MediaItem {
        ...
        serviceScope.launch {
            kotlinx.coroutines.flow.combine(sleepTimerManager.isActive, sleepTimerManager.remainingTimeMs) { isActive, remaining -> Pair(isActive, remaining) }.collect { (isActive, remaining) ->
                if (isActive && remaining != null) {
                    val timeString = com.sonza.app.util.TimeUtil.formatPosition(remaining)
                    ...
                    val notification = androidx.core.app.NotificationCompat.Builder(this@MusicPlayerService, channelId)
                        .setSmallIcon(com.sonza.app.R.drawable.ic_music_note)
                        .setContentTitle("Sleep Timer Active")
                        .setContentText("Stopping audio in $timeString")
                        .setOnlyAlertOnce(true)
                        .setOngoing(true)
                        .addAction(com.sonza.app.R.drawable.ic_music_note, "Cancel", pendingCancelIntent)
                        .setColor(androidx.core.content.ContextCompat.getColor(this@MusicPlayerService, com.sonza.app.R.color.black))
                        .build()
