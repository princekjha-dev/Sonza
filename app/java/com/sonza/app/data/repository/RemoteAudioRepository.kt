...
import com.sonza.app.util.encodeUrl
import com.sonza.app.util.decodeHtml
import com.sonza.app.util.toHighResImage
import com.sonza.app.data.repository.remote.RemoteConstants
import com.sonza.app.core.model.AppResult
import com.sonza.app.core.model.getOrDefault
...
class RemoteAudioRepository @Inject constructor(
    ...
    private val apiService: com.sonza.app.data.repository.remote.RemoteAudioApiService,
    private val playlistApiService: com.sonza.app.data.repository.remote.HqAudioPlaylistApiService,
     ...
     * [com.sonza.app.core.model.AppError]. Failures are also reported to telemetry.
     *
    ...
    suspend fun searchResult(query: String): AppResult<List<Song>> = withContext(Dispatchers.IO) {
        ...
        if (isRateLimited()) {
            ...
            return@withContext if (stale != null) {
                ...
                AppResult.Failure(com.sonza.app.core.model.AppError.RateLimited("backoff"))
            }
        }
        ...
        try {
            ...
            if (rawCount > 0 && songs.isEmpty()) {
                ...
                Telemetry.report("search", "remoteaudio", com.sonza.app.core.model.AppError.Parse("$rawCount raw, 0 parsed"))
            }
            ...
            if (error is com.sonza.app.core.model.AppError.RateLimited) noteRateLimited()
     ...
     */
    suspend fun searchArtists(query: String): List<com.sonza.app.core.model.Artist> = withContext(Dispatchers.IO) {
        if (isRateLimited()) {
        ...
        try {
            ...
            response.data?.results.orEmpty().mapNotNull { dto ->
                ...
                com.sonza.app.core.model.Artist(
                    id = id,
     ...
     */
    suspend fun searchPlaylists(query: String): List<com.sonza.app.core.model.Playlist> = withContext(Dispatchers.IO) {
        if (isRateLimited()) {
    ...
    private fun com.sonza.app.data.repository.remote.PlaylistSearchItem.toPlaylist(
        author: String
    ): Playlist? {
     ...
     */
    suspend fun getArtist(artistId: String): com.sonza.app.core.model.Artist? = withContext(Dispatchers.IO) {
        if (isRateLimited()) {
            ...
            try {
                getHomeSections().forEach { section ->
                    section.items.forEach { item ->
                        if (item is com.sonza.app.core.model.HomeItem.SongItem) pool.add(item.song)
                    }
                }
            } catch (e: Exception) {
     ...
     */
    suspend fun getHomeSections(): List<com.sonza.app.core.model.HomeSection> = withContext(Dispatchers.IO) {
        val sections = mutableListOf<com.sonza.app.core.model.HomeSection>()
        ...
        try {
            ...
            fun parseHomeItems(jsonArray: com.google.gson.JsonArray): List<com.sonza.app.core.model.HomeItem> {
                return jsonArray.mapNotNull { element ->
                    ...
                    when (type) {
                        "song" -> {
                            parseSong(obj)?.let { com.sonza.app.core.model.HomeItem.SongItem(it) }
                        }
                        "album" -> {
                            ...
                            com.sonza.app.core.model.HomeItem.AlbumItem(
                                com.sonza.app.core.model.Album(id, title.decodeHtml(), artist.decodeHtml(), image, year)
                            )
                        }
                        "playlist" -> {
                            val id = obj.get("id")?.asString?.takeIf { it.isNotBlank() } ?: return@mapNotNull null
                            ...
                            com.sonza.app.core.model.HomeItem.PlaylistItem(
                                com.sonza.app.core.model.PlaylistDisplayItem(id, title.decodeHtml(), "", "Featured", image, count)
                            )
                        }
                        "chart" -> {
                            val id = obj.get("id")?.asString?.takeIf { it.isNotBlank() } ?: return@mapNotNull null
                            ...
                            com.sonza.app.core.model.HomeItem.PlaylistItem(
                                com.sonza.app.core.model.PlaylistDisplayItem(id, title.decodeHtml(), "", "Top Charts", image, count)
                            )
                        }
                        "radio_station" -> {
                            val id = obj.get("id")?.asString?.takeIf { it.isNotBlank() } ?: return@mapNotNull null
                            ...
                            com.sonza.app.core.model.HomeItem.PlaylistItem(
                                com.sonza.app.core.model.PlaylistDisplayItem("radio_$id", title.decodeHtml(), "", "Radio", image, 0)
                            )
                        }
                        "artist" -> {
                             val id = obj.get("id")?.asString?.takeIf { it.isNotBlank() } ?: return@mapNotNull null
                            val title = obj.get("title")?.asString ?: obj.get("name")?.asString ?: ""
                            val image = obj.get("image")?.asString?.toHighResImage()
                             com.sonza.app.core.model.HomeItem.ArtistItem(
                                com.sonza.app.core.model.Artist(id, title.decodeHtml(), image)
                            )
                        }
                        else -> null
                    }
            ...
            if (json.has("charts")) {
                ...
                if (items.isNotEmpty()) {
                    sections.add(com.sonza.app.core.model.HomeSection("Top Charts 📊", items))
                }
            }
            ...
            if (json.has("new_albums")) {
                ...
                if (items.isNotEmpty()) {
                    sections.add(com.sonza.app.core.model.HomeSection("New Releases 🆕", items))
                }
            }
            ...
            if (json.has("browse_discover")) {
                ...
                discoverList.forEach { element ->
                    ...
                    if (!title.isNullOrBlank() && data != null && data.size() > 0) {
                        ...
                        if (items.isNotEmpty()) {
                            sections.add(com.sonza.app.core.model.HomeSection(title.decodeHtml(), items))
                        }
                    }
                }
            }
            ...
            if (json.has("top_playlists")) {
                ...
                if (items.isNotEmpty()) {
                    sections.add(com.sonza.app.core.model.HomeSection("Featured Playlists 🎧", items))
                }
            }
     ...
     */
    private suspend fun fetchStaticHomeSections(): List<com.sonza.app.core.model.HomeSection> = withContext(Dispatchers.IO) {
        val sections = mutableListOf<com.sonza.app.core.model.HomeSection>()
        ...
        try {
            ...
            try {
                ...
                if (chartsJson.isJsonArray && chartsJson.asJsonArray.size() > 0) {
                    ...
                    if (chartItems.isNotEmpty()) {
                        sections.add(com.sonza.app.core.model.HomeSection("Top Charts 📊", chartItems))
                    }
                }
            ...
            try {
                ...
                if (albumsList != null && albumsList.size() > 0) {
                    ...
                    if (albumItems.isNotEmpty()) {
                        sections.add(com.sonza.app.core.model.HomeSection("New Releases 🆕", albumItems))
                    }
                }
            ...
            try {
                ...
                if (plList != null && plList.size() > 0) {
                    ...
                    if (playlistItems.isNotEmpty()) {
                        sections.add(com.sonza.app.core.model.HomeSection("Featured Playlists 🎧", playlistItems))
                    }
                }
            ...
            try {
                ...
                if (artistResults != null && artistResults.size() > 0) {
                    ...
                    if (artistItems.isNotEmpty()) {
                        sections.add(com.sonza.app.core.model.HomeSection("Top Artists 🎤", artistItems))
                    }
                }
            ...
            try {
                ...
                if (albumArray != null && albumArray.size() > 0) {
                    ...
                    if (albumItems.isNotEmpty()) {
                        sections.add(com.sonza.app.core.model.HomeSection("Popular Albums 💿", albumItems))
                    }
                }
     ...
     */
    suspend fun getFeaturedPlaylists(): List<com.sonza.app.core.model.PlaylistDisplayItem> = withContext(Dispatchers.IO) {
        val playlists = mutableListOf<com.sonza.app.core.model.PlaylistDisplayItem>()
    ...
    private fun parseSongDto(dto: com.sonza.app.data.repository.remote.RemoteAudioSongDto): Song? {
        return try {
            ...
            val metadata = com.sonza.app.core.model.RemoteAudioMetadata(
                ...
                artists = dto.artists?.all?.mapNotNull { artist ->
                    ...
                    com.sonza.app.core.model.ArtistCreditInfo(
                        name = name.decodeHtml(),
    ...
    private fun parseHqAudioPlaylistSong(
        dto: com.sonza.app.data.repository.remote.HqAudioPlaylistSong,
    ...
    suspend fun searchAll(query: String): SearchResults = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            // Parse Albums
            val albums = emptyList<com.sonza.app.core.model.Album>()
...
data class SearchResults(
    ...
    val albums: List<com.sonza.app.core.model.Album> = emptyList(),
    val artists: List<com.sonza.app.core.model.Artist> = emptyList(),
