package com.sonza.app.data.repository.youtube.streaming.potoken
...
/**
 * Hidden-WebView BotGuard PoToken generator.
 *
 * Faithful port of Metrolist's `PoTokenWebView` (itself adapted from NewPipe /
 * LuanRT BgUtils). It runs YouTube's BotGuard VM inside a WebView whose own
 * network access is blocked — every HTTP call goes through the supplied OkHttp
 * client instead. The accompanying `assets/po_token.html` + [JavaScriptUtil]
 * codec helpers are version-sensitive and must stay byte-exact with the
 * reference.
 */
class PoTokenWebView private constructor(
    ...
    companion object {
        ...
        suspend fun getNewPoTokenGenerator(context: Context, httpClient: OkHttpClient): PoTokenWebView {
            return withContext(Dispatchers.Main) {
                suspendCancellableCoroutine { cont ->
                    val potWv = PoTokenWebView(context, httpClient, cont)
