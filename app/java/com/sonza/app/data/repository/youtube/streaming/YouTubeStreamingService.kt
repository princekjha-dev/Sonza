package com.sonza.app.data.repository.youtube.streaming

import android.util.LruCache
import com.sonza.app.data.SessionManager
import com.sonza.app.core.model.Song
import com.sonza.app.core.model.AppError
...
class YouTubeStreamingService @Inject constructor(
            ...
            dedupScope.async {
                try {
                    ...
                    val itQuality = if (forceLow) {
                        com.sonza.app.core.model.AudioQuality.LOW
                    } else {
    ...
    private suspend fun resolveStreamWithUrl(streamUrl: String, videoId: String, forceLow: Boolean = false): String? {
        ...
        return retryWithBackoff(opTag = "audio[$videoId@$host]") {
            ...
            if (audioQuality == com.sonza.app.core.model.AudioQuality.AUTO) {
                audioQuality = if (networkMonitor.isOnWifi()) {
                    com.sonza.app.core.model.AudioQuality.MEDIUM
                } else {
                    com.sonza.app.core.model.AudioQuality.LOW
                }
            ...
            val targetBitrate = when (audioQuality) {
                com.sonza.app.core.model.AudioQuality.LOW -> 70
                com.sonza.app.core.model.AudioQuality.MEDIUM -> 160
                com.sonza.app.core.model.AudioQuality.HIGH -> 512
    ...
    suspend fun getVideoStreamUrl(videoId: String, quality: com.sonza.app.core.model.VideoQuality? = null, forceLow: Boolean = false): String? = withContext(Dispatchers.IO) {
        getVideoStreamResult(videoId, quality, forceLow)?.videoUrl
    }
    
    suspend fun getVideoStreamResult(videoId: String, quality: com.sonza.app.core.model.VideoQuality? = null, forceLow: Boolean = false): VideoStreamResult? = withContext(Dispatchers.IO) {
        val targetQuality = if (forceLow) {
            com.sonza.app.core.model.VideoQuality.LOW
        } else {
    ...
    private suspend fun resolveVideoWithUrl(
        streamUrl: String,
        videoId: String,
        targetQuality: com.sonza.app.core.model.VideoQuality
    ): VideoStreamResult? {
        val videoCacheKey = "video_${videoId}_${targetQuality.name}"
        val audioCacheKey = "video_audio_${videoId}_${targetQuality.name}"
        val host = streamUrl.substringAfter("//").substringBefore("/")
        ...
        return retryWithBackoff(opTag = "video[$videoId@$host/$targetQuality]") {
            ...
            // Adaptive logic for AUTO quality
            if (quality == com.sonza.app.core.model.VideoQuality.AUTO) {
                quality = if (networkMonitor.isOnWifi()) {
                    com.sonza.app.core.model.VideoQuality.MEDIUM
                } else {
                    com.sonza.app.core.model.VideoQuality.LOW
                }
    ...
    fun clearCacheFor(videoId: String) {
        ...
        com.sonza.app.core.model.VideoQuality.entries.forEach { 
